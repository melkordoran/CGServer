...
#define AW_BUILD                    101     // all AW 5.2 and later, backward compatble to build 65
...
#define AW_MAX_USER_NAME_LENGTH     20
#define AW_MIN_USER_NAME_LENGTH     2
#define AW_MAX_PASSWORD_LENGTH      16
#define AW_MIN_PASSWORD_LENGTH      4
#define AW_MAX_EMAIL_LENGTH         64
#define AW_MAX_WORLD_NAME_LENGTH    16
...
#define AW_OBJECT_VERSION_CLOTH       1
...
typedef enum {
  AW_CHAT_CHANNEL_NONE         = 0x00000000,  // backward compatibility, defaults to LOCAL
  AW_CHAT_CHANNEL_LOCAL        = 0x00000001,  // LOCAL neighborhood
  AW_CHAT_CHANNEL_GLOBAL       = 0x00000002,  // GLOBAL / worldwide
  AW_CHAT_CHANNEL_WORLD1       = 0x00000010,  // WORLD defined #1
  AW_CHAT_CHANNEL_WORLD2       = 0x00000020,  // WORLD defined #2
  AW_CHAT_CHANNEL_WORLD3       = 0x00000040,  // WORLD defined #3
  AW_CHAT_CHANNEL_WORLD4       = 0x00000080,  // WORLD defined #4
  AW_CHAT_CHANNEL_WORLD5       = 0x00000100,  // WORLD defined #5
  AW_CHAT_CHANNEL_UNIVERSE     = 0x01000000,  // UNIVERSAL
} AW_CHAT_CHANNELS;
...
typedef enum
{
  AW_ZONE_FLAG_WATER              = 0x001,
  AW_ZONE_FLAG_BLOCK_PARTICLES    = 0x002,
  AW_ZONE_FLAG_BLOCK_LIGHTS       = 0x004,
  AW_ZONE_FLAG_BLOCK_WORLD_LIGHT  = 0x008,
  AW_ZONE_FLAG_BLOCK_CHAT         = 0x010,
  AW_ZONE_FLAG_VISIBLE            = 0x020,
  AW_ZONE_FLAG_BLOCK_JOIN         = 0x040,
  AW_ZONE_FLAG_VOIP               = 0x080,
  AW_ZONE_FLAG_BLOCK_AUDIO        = 0x100,
} AW_ZONE_FLAGS;
...
typedef enum {
  ...
  AW_PARTICLE_FLAG_APPLY_ROTATION = 0x80,
} AW_PARTICLE_FLAGS;
...
typedef enum {
  AW_CLOTH_FLAG_GRAVITY             = 0x0001,
  AW_CLOTH_FLAG_STATIC              = 0x0002,
  AW_CLOTH_FLAG_COMDAMPING          = 0x0004,
  AW_CLOTH_FLAG_BENDING_ORTHO       = 0x0008,
  AW_CLOTH_FLAG_SELFCOLLISION       = 0x0010,
  AW_CLOTH_FLAG_COLLISION_TWO_WAY   = 0x0020,
  AW_CLOTH_FLAG_ATTACHMENT_TWO_WAY  = 0x0040,
  AW_CLOTH_FLAG_ATTACHMENT_TEARABLE = 0x0080,
  AW_CLOTH_FLAG_TEARABLE            = 0x0100,
  AW_CLOTH_FLAG_TEARABLE_DIAGONAL   = 0x0200, // fabric only
  AW_CLOTH_FLAG_PRESSURE            = 0x0400, // closed convex hulls only
  AW_CLOTH_FLAG_ADHERE              = 0x0800,
  AW_CLOTH_FLAG_UNTANGLING          = 0x1000,
  AW_CLOTH_FLAG_HARD_STRETCH_LIMIT  = 0x2000,
  AW_CLOTH_FLAG_TEX_NO_MIRROR       = 0x4000,
} AW_CLOTH_FLAGS;
...
typedef enum {
  AW_XFER_TYPE_FILE     = 0,
  AW_XFER_TYPE_IMAGE    = 1,
  AW_XFER_TYPE_DOCUMENT = 2,
  AW_XFER_TYPE_MEDIA    = 3,
  AW_XFER_TYPE_STORE    = 4,		// file to store
  AW_XFER_TYPE_RETRIEVE = 5,		// file to retrieve
} AW_XFER_TYPES;

// AW_LASER_BEAM_SOURCE_TYPE
typedef enum {
  AW_LASER_BEAM_SOURCE_TYPE_NONE    = 0,
  AW_LASER_BEAM_SOURCE_TYPE_AVATAR  = 1,
  AW_LASER_BEAM_SOURCE_TYPE_OBJECT  = 2,
  AW_LASER_BEAM_SOURCE_TYPE_COORDS  = 3,
} AW_LASER_BEAM_SOURCE_TYPES;

// AW_LASER_BEAM_TARGET_TYPE
typedef enum {
  AW_LASER_BEAM_TARGET_TYPE_NONE    = 0,
  AW_LASER_BEAM_TARGET_TYPE_AVATAR  = 1,
  AW_LASER_BEAM_TARGET_TYPE_OBJECT  = 2,
  AW_LASER_BEAM_TARGET_TYPE_COORDS  = 3,
} AW_LASER_BEAM_TARGET_TYPES;

// AW_LASER_BEAM_STYLE
typedef enum {
  AW_LASER_BEAM_STYLE_LINE          = 0,
  AW_LASER_BEAM_STYLE_PARTICLE      = 1,
} AW_LASER_BEAM_STYLES;

// AW_SHOPITEM_TYPE
typedef enum {
  AW_SHOPITEM_TYPE_CAV              = 0,
} AW_SHOPITEM_TYPES;
...
// AW_OBJECT_TYPE_CLOTH
typedef struct aw_object_data_cloth {
  unsigned char         version;          // structure version (set by SDK)
  unsigned char         type;             // AW_CLOTH_TYPES
  unsigned char         shape;            // AW_CLOTH_FABRIC_SHAPES + AW_CLOTH_METAL_SHAPES
  unsigned int          flags;            // AW_CLOTH_FLAGS
  unsigned char         anchor_type;      // AW_CLOTH_ANCHORS
  unsigned char         anchor_pos;       // AW_CLOTH_ANCHOR_POS, for fabric
  // pose definitions
  aw_type_vector        anchor_offset;    // offset to anchor, 'local pose'
  aw_type_vector        orientation;      // normalized rotation angles
  // sizes
  aw_type_vector        dimension;        // dimension for fabric, scaling for models
  unsigned char         resolution;       // fabric only, divisor to size
  // render options
  float                 opacity;
  unsigned int          color;            // COLORREF
  float                 time_till_static; // time in seconds
  // physics behavior
  float                 bending_stiffness;
  float                 stretching_stiffness;
  float                 compression_limit;
  float                 compression_stiffness;
  float                 hard_stretch_limit;
  float                 damping_coeff;
  float                 collision_response;
  float                 tear_factor;
  float                 attachment_response;
  float                 attachment_tear_factor;
  float                 min_adhere_velocity;
  // physics properties
  float                 density;          // mass = 1/3 * triangleArea * density
  float                 thickness;
  float                 friction;
  float                 pressure;         // closed convex hull models only
  aw_type_vector        wind;             // wind acceleration on vertex normals
  aw_type_vector        valid_bounds;     // triangles outside these bounds are not simulated
  // AW_CLOTH_TYPE_METAL_SHAPE only:
  float                 impulse_threshold;
  float                 penetration_depth;
  float                 max_deformation_distance;
  // appendices
  unsigned char         name_len;         // length of 1st string in str_data, the object's own name
  unsigned char         anchor_len;       // length of 2nd string in str_data, anchor name
  unsigned char         texture_len;      // length of 3rd string in str_data, texture name
  unsigned char         model_len;        // length of 4th string in str_data, model name
  char                  reserved[32];     // Bytes reserverd for future use
  // note: increase the strucure's total size
  // by the length of the strings
  // when allocating memory for this structure!
  // (str_data is the reference to the first byte)
  char                  str_data[1];
} aw_object_data_cloth;
...
AWAPI int   aw_createW (const wchar_t* domain, int port, void** instance);
...
AWAPI int   aw_enterW (const wchar_t* world);
...
AWAPI int   aw_sayW (const wchar_t* message);
...
AWAPI int   aw_world_attribute_setW (int attribute, wchar_t* value);
AWAPI int   aw_world_attribute_get (int attribute, int* read_only, char* value);
AWAPI int   aw_world_attribute_getW (int attribute, int* read_only, wchar_t* value);
...
#if defined (UNICODE) && !defined (AW_NO_FUNCTION_MAPPING)

// Map to Wide version of functions
// These mappings are meant for applications compiling with this SDK,
// not for the SDK itsself.
#define aw_create                     aw_createW
#define aw_string                     aw_stringW
#define aw_string_set                 aw_string_setW
#define aw_enter                      aw_enterW
#define aw_say                        aw_sayW
#define aw_say_channel                aw_say_channelW
#define aw_whisper                    aw_whisperW
#define aw_citizen_attributes_by_name aw_citizen_attributes_by_nameW
#ifdef AW_BROWSER
#define aw_shopitem_by_desc           aw_shopitem_by_descW
#endif
#define aw_license_attributes         aw_license_attributesW
#define aw_license_delete             aw_license_deleteW
#define aw_url_send                   aw_url_sendW
#define aw_url_click                  aw_url_clickW
#define aw_world_attribute_set        aw_world_attribute_setW
#define aw_world_attribute_get        aw_world_attribute_getW
#define aw_avatar_location            aw_avatar_locationW
#define aw_check_right                aw_check_rightW
#define aw_check_right_all            aw_check_right_allW
#define aw_server_admin               aw_server_adminW

#define aw_contact_add_by_name        aw_contact_add_by_nameW
#define aw_telegram_send              aw_telegram_sendW
#define aw_tunnel_set                 aw_tunnel_setW
#define aw_proxy_set                  aw_proxy_setW
#define aw_proxy_set_login            aw_proxy_set_loginW
#define aw_voip_group_set             aw_voip_group_setW
#define aw_voipcast_set               aw_voipcast_setW
#define aw_xfer_request_show          aw_xfer_request_showW

#endif  // UNICODE && !AW_NO_FUNCTION_MAPPING
